...
interface LinkButtonProps {
    ...
    hidden?: boolean;
    bottom?: boolean;
    color: "white" | "orange" | "orangeWhite" | "green" | "purple";
    back?: boolean;
    big?: boolean;
    className?: string;
}
...
export default function LinkButton({href, text, center, hidden, bottom, color, className, big, back}: LinkButtonProps) {
    return (
        <Link href={href} className={`${hidden ? "hidden lg:flex" : ""} px-7 py-4 rounded-4xl 
            ...
            ${color == "orangeWhite" ? "bg-miau-orange text-background hover:bg-miau-green active:bg-miau-light-green font-bold" : ""}
            ${color == "purple" ? "bg-miau-purple text-background hover:bg-miau-green active:bg-miau-light-green font-bold" : ""}
            ${className ? className : ""} ${big ? "text-lg lg:text-xl font-semibold" : ""}`}>
            {text}
        </Link>
    );
}